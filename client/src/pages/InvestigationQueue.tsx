...
export default function InvestigationQueue() {
  ...
  return <DashboardLayout><div className="container space-y-6 pb-12">
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {[{label:"Open investigations", value:(investigations.data ?? []).filter(i => i.status === "OPEN").length, icon:ClipboardList, accent:"accent-strip-saffron"}, {label:"Closed decisions", value:(investigations.data ?? []).filter(i => i.status === "CLOSED").length, icon:ShieldAlert, accent:"accent-strip-green"}, {label:"Common-cause signals", value:commonCauseSignals.length, icon:Users, accent:"accent-strip-navy"}, {label:"High-risk components", value:summary.data?.highRisk ?? "—", icon:AlertTriangle, accent:"border-l-[3px] border-l-destructive"}].map(item => <Card key={item.label} className={`blueprint-panel ${item.accent}`}><CardContent className="p-4"><div className="flex items-center justify-between text-muted-foreground"><span className="blueprint-label">{item.label}</span><item.icon className="h-4 w-4 text-primary" /></div><div className="metric-value mt-3 text-3xl font-semibold">{item.value}</div></CardContent></Card>)}
    </section>
